...
class AdminProcess {
    ...
    function AdminProcess() {
        ...
        /* Admin submitted print flight details form */ else if (isset($_POST['subprintflight'])) {
            $this->procPrintFlightDetails();
        }
        /* Admin submitted print customer details form */ else if (isset($_POST['subprintcust'])) {
            $this->procPrintCustDetails();
        }
    ...
    /**
     * procPrintFlightDetails - Prints the details of all customers
     * with a flight booked on the selected date and flight.
     */
    function procPrintFlightDetails() {
        global $session, $database, $form;

        $subflight_id = $_POST['flight_id'];

        /* Date error checking */
        $subdate = $_POST['printdate'];
        $field = "printdate";  //Use field name for date
        ...
        if (date('l', $tstamp) == 'Sunday') {
            $form->setError($field, "* We don't fly on Sundays");
        }
        ...
        $result = $database->getFlightDetails($subdate, $subflight_id);
        if (!$result) {
            die("getFlightDetails failed");
        } else {
            $_SESSION['printdate'] = $result['flight_date']; //date to be printed
            $_SESSION['printalloc'] = $result['seats_alloc']; //number of seats allocated
            $_SESSION['printid'] = $result['flightid']; //flight id
            $_SESSION['printdeptime'] = $result['departure_time']; //departure time
            $_SESSION['printarrtime'] = $result['arrival_time']; //arrival time
            $_SESSION['printorigin'] = $result['origin']; //origin
            $_SESSION['printdest'] = $result['destination']; //destination
            header("Location: " . $session->referrer);
        }
    }

    /**
     * procPrintCustDetails - Attempts to get customer details
     * for the given flight route and date.
     */
    function procPrintCustDetails() {
        global $session, $database, $form;

        $subflight_id = $_POST['flight_id'];

        /* Date error checking */
        $subdate = $_POST['custdate'];
        $field = "custdate";  //Use field name for date
        if (!$subdate || strlen($subdate = trim($subdate)) == 0) {
            $form->setError($field, "* Date not entered");
        } else {
            /* Check if valid date */
        ...
        if ($form->num_errors > 0) {
            ...
            $_SESSION['custsuccess'] = true;
            $_SESSION['custflightdate'] = $subdate;
            $_SESSION['custflightid'] = $subflight_id;
            header("Location: " . $session->referrer);
        }
    }
